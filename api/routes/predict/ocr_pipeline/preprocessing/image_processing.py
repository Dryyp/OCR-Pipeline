...
def dog(img, k_size, sigma_1, sigma_2):    
    ...
    # zero-normalize scaling kernel with scaling factor 1.0
...
def roi_enhance(img):
    ...
    dog_k_size, dog_sigma_1, dog_sigma_2 = 10, 100, 0

    # Difference of Gaussian (DoG)
    img = dog(img, dog_k_size, dog_sigma_1, dog_sigma_2)
    return img
...
def preprocess(img):
    ...
    img = negative(img)
    # Contrast Stretch
    img = contrast_stretch(img, cs_black_per, cs_white_per)
    # Gaussian Blur
    img = fast_gaussian_blur(img, gauss_k_size, gauss_sigma)
    # Gamma Correction
    img = gamma(img, gamma_value)
